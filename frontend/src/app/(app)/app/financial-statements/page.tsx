...
type Organization = { id: string; name: string; national_id: string };
type FiscalYear = { id: string; title: string; start_date?: string; end_date?: string };
...
export default function FinancialStatementsPage() {
  ...
  async function createOrganization() {
    const name = window.prompt("نام شرکت یا مؤسسه را وارد کنید:"); if (!name) return;
    const nationalId = window.prompt("شناسه ملی را وارد کنید:") ?? ""; setBusy("organization");
    try { const item = await api<Organization>("api/v1/financial-statements/organizations", { method: "POST", body: JSON.stringify({ name, national_id: nationalId }) }); await loadBase(); setOrganizationId(item.id); setMessage("شرکت ثبت شد."); } catch (error) { showError(error); } finally { setBusy(""); }
  }
  async function createYear() {
    if (!organizationId) return setMessage("ابتدا شرکت را انتخاب کنید.");
    const title = window.prompt("عنوان سال مالی، مانند ۱۴۰۴:"); if (!title) return; setBusy("year");
    try { const item = await api<FiscalYear>("api/v1/financial-statements/fiscal-years", { method: "POST", body: JSON.stringify({ organization_id: organizationId, title }) }); setYears((old) => [item, ...old]); setYearId(item.id); setMessage("سال مالی ثبت شد."); } catch (error) { showError(error); } finally { setBusy(""); }
  }
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
      <div className="space-y-6">
        ...
        {selectedImport && <Card><CardHeader><CardTitle>۲. کنترل و نگاشت حساب‌ها</CardTitle></CardHeader><CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-3 rounded-xl bg-slate-50 p-4 text-sm"><b>{selectedImport.original_filename}</b><span>{statusLabel[selectedImport.status] ?? selectedImport.status}</span><span>پوشش نگاشت: {number(selectedImport.mapping_coverage)}٪</span><span>کل حساب‌ها: {number(selectedImport.total_accounts)}</span>{selectedImport.status === "uploaded" || selectedImport.status === "failed" ? <Button size="sm" onClick={parse} disabled={busy === "parse"}>{busy === "parse" && <Loader2 className="animate-spin" />} پردازش فایل</Button> : null}</div>
