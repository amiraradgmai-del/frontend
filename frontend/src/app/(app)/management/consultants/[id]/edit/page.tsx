      ...
      await api(`api/v1/consultations/manage/profiles/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          contract_start: form.contract_start
            ? new Date(form.contract_start).toISOString()
            : null,
          contract_end: form.contract_end
            ? new Date(form.contract_end).toISOString()
            : null,
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm font-bold text-blue-600">مدیریت مشاوران</p>
          <h1 className="mt-1 text-3xl font-black">ویرایش کامل مشاور</h1>
      ...
      <form onSubmit={submit} className="space-y-5">
        <Section title="اطلاعات حرفه‌ای">
          ...
          <Field label="هزینه هر جلسه (تومان)">
            <Input
              type="number"
              min={0}
          ...
          <Field wide label="سوابق و مدارک">
            <Textarea
              className="min-h-28"
          ...
          <Field label="حوزه‌های تخصصی (با ویرگول جدا کنید)">
            <Input
              value={form.specialties.join("، ")}
              onChange={(e) =>
                setForm({ ...form, specialties: split(e.target.value) })
              }
            />
          </Field>
          <Field label="مهارت‌ها (با ویرگول جدا کنید)">
            <Input
              value={form.skills.join("، ")}
              onChange={(e) =>
                setForm({ ...form, skills: split(e.target.value) })
              }
            />
          </Field>
          <Field wide label="آدرس دفتر">
            <Input
        ...
        <Section title="وضعیت و شیوه ارائه خدمت">
        ...
        <Section title="اطلاعات بانکی">
          ...
          <Field label="شماره شبا">
            <Input
              ...
              onChange={(e) =>
                setForm({ ...form, bank_iban: e.target.value.toUpperCase() })
              }
            />
          </Field>
        </Section>
        <Section title="قرارداد همکاری">
          ...
          <Field label="وضعیت قرارداد">
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, contract_status: e.target.value })
              }
            >
              <option value="not_set">ثبت نشده</option>
          ...
          <Field label="شروع قرارداد (شمسی)">
            <PersianDateInput value={form.contract_start?.slice(0, 10) ?? ""} onChange={(value) => setForm({ ...form, contract_start: value || null })} />
          </Field>
          <Field label="پایان قرارداد (شمسی)">
            <PersianDateInput value={form.contract_end?.slice(0, 10) ?? ""} onChange={(value) => setForm({ ...form, contract_end: value || null })} />
          </Field>
        </Section>
        <Button className="w-full py-6 text-base" disabled={saving}>
          <Save /> {saving ? "در حال ذخیره…" : "ذخیره تمام تغییرات"}
        </Button>
      </form>
    </div>
  );
...
function Field({
  ...
  return (
    <label
      className={`space-y-2 text-sm font-bold ${wide ? "sm:col-span-2" : ""}`}
    >
      <span>{label}</span>
      {children}
    </label>
...
function Checks({
  ...
  form: Profile;
  setForm: (value: Profile) => void;
}) {
  return (
    <>
      {[
        ["is_verified", "صلاحیت تأییدشده"],
        ["is_available", "آماده پذیرش"],
        ["is_online", "مشاوره آنلاین"],
        ["offers_in_person", "مشاوره حضوری"],
      ].map(([key, label]) => (
        <label
          key={key}
        ...
        >
          <input
            type="checkbox"
            checked={Boolean(form[key as keyof Profile])}
