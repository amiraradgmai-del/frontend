...
export default function ChatPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setMessages((items) => [...items, { id: response.message_id, role: "assistant", content: response.answer, confidence: response.confidence, needs_expert: response.needs_expert, disclaimer: response.disclaimer, created_at: new Date().toISOString(), citations: [], fullCitations: response.citations, answerBasis: response.answer_basis, sourceNotice: response.source_notice }]);
    } catch (requestError) {
