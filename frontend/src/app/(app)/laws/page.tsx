...
type Law = { id: string; law_name: string; chapter: string; article_number: string; official_text: string; plain_language_summary: string; category_title: string | null; publication_date: string | null; effective_date: string | null; suggested_questions: SuggestedQuestion[] };
    ...
    <Card className="border-sky-100 bg-white/95 shadow-sm"><CardContent className="pt-6">
      <form onSubmit={submit} className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_260px_auto]">
        ...
        <select aria-label="دسته‌بندی قانون" value={category} onChange={(event) => { setCategory(event.target.value); void load(query, event.target.value); }} className="h-10 rounded-lg border border-input bg-white px-3 text-sm">
          ...
          {categories.map((item) => <option key={item.id} value={item.id}>{item.title} ({item.law_count.toLocaleString("fa-IR")})</option>)}
        </select>
      ...
      {laws.map((law) => {
        ...
        return <article key={law.id} className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm transition hover:border-blue-200 hover:shadow-md">
          <button type="button" aria-expanded={expanded} onClick={() => { setExpandedId(expanded ? "" : law.id); setOpenQuestion(""); }} className="w-full p-5 text-right sm:p-6">
            <div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><Badge className="text-sm">ماده {law.article_number}</Badge>{law.category_title && <Badge variant="outline">{law.category_title}</Badge>}</div><h2 className="mt-4 text-lg font-black text-slate-900">{law.suggested_questions[0]?.question || `ماده ${law.article_number} چه حکمی دارد؟`}</h2><p className="mt-2 text-xs font-medium text-slate-500">منبع: {law.law_name}{law.chapter ? ` · ${law.chapter}` : ""}</p><p className="mt-3 line-clamp-2 text-sm leading-7 text-slate-600">{law.plain_language_summary}</p></div><ChevronDown className={`mt-2 size-5 shrink-0 text-blue-600 transition ${expanded ? "rotate-180" : ""}`} /></div>
          </button>
          {expanded && <div className="border-t border-slate-100 bg-slate-50/50 p-5 sm:p-6">
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5"><p className="mb-3 text-xs font-black text-emerald-800">خلاصه ساده و قابل‌فهم</p><p className="whitespace-pre-line text-sm leading-8 text-emerald-950">{law.plain_language_summary}</p><p className="mt-3 text-xs text-emerald-800">این خلاصه برای فهم سریع است؛ در تصمیم حقوقی، متن رسمی زیر ملاک است.</p></div><div className="mt-4 rounded-2xl border bg-white p-5"><p className="mb-3 text-xs font-black text-blue-700">متن رسمی ماده</p><p className="whitespace-pre-line text-sm leading-9 text-slate-800">{law.official_text}</p></div>
