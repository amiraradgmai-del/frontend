      ...
      <section className="px-5 py-10">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-start">
          <div className="space-y-6">
            ...
            {visiblePosts.map((post) => <article key={post.slug} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{post.cover_image_url ? <div className="h-64 w-full bg-cover bg-center sm:h-80" style={{ backgroundImage: `url(${post.cover_image_url})` }} /> : <div className="h-64 w-full bg-gradient-to-br from-blue-100 to-cyan-50 sm:h-80" />}<div className="p-6"><p className="text-xs font-bold text-blue-600">{post.category || "مقاله"}</p><h2 className="mt-2 text-2xl font-black leading-10"><Link href={`/blog/${post.slug}`} className="hover:text-blue-700">{post.title}</Link></h2>{post.published_at && <time className="mt-2 block text-xs text-slate-400">انتشار در {new Date(post.published_at).toLocaleDateString("fa-IR")}</time>}<p className="mt-4 line-clamp-3 text-sm leading-7 text-slate-600">{post.excerpt}</p><Link href={`/blog/${post.slug}`} className="mt-5 inline-flex rounded-lg border border-orange-500 px-4 py-2 text-sm font-bold text-orange-600 transition hover:bg-orange-50">ادامه ←</Link></div></article>)}
