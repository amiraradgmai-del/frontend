...
function readValue(value:string){const date=value?new Date(value):null;return date&&!Number.isNaN(date.getTime())?toJalali(date.getFullYear(),date.getMonth()+1,date.getDate()):[0,0,0];}
...
export function PersianDateInput({value,onChange,required=false,includeTime=false}:{value:string;onChange:(value:string)=>void;required?:boolean;includeTime?:boolean}){
  const [parts,setParts]=useState<number[]>(()=>readValue(value)); const [time,setTime]=useState(value.includes("T")?value.slice(11,16):"09:00");
  const [previousValue,setPreviousValue]=useState(value);
  if(value!==previousValue){setPreviousValue(value);setParts(readValue(value));setTime(value.includes("T")?value.slice(11,16):"09:00");}
